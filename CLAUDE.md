...
|---|---|---|
| **GHL** | ✅ Sí — **EN ENFORCE desde 2026-08-25** | Dos firmas en paralelo: `x-ghl-signature` (**Ed25519**, 64 bytes base64, clave pública universal de GHL — la vigente) y `x-wh-signature` (RSA-SHA256, **deprecada por GHL el 2026-09-01**). El verifier combinado (`apps/motor-agente/src/lib/webhook-verify-marketplace.ts`) prefiere Ed25519 > RSA > HMAC legado, discriminando por la forma del header. Claves públicas en `GHL_WEBHOOK_ED25519_PUBLIC_KEY_PEM` / `GHL_WEBHOOK_PUBLIC_KEY_PEM` (en el VPS van en UNA línea con `\n` literales; `env.ts` los convierte — el `env_file` de docker compose no admite multilínea). Verificado en producción: webhook real OK por ed25519 + request sin firma → 401. |
| **YCloud** | ✅ Sí | Header `YCloud-Signature: t=<unix_seconds>,s=<hmac_sha256_hex>`. Signed payload `{ts}.{rawBody}`. Secret se obtiene del panel YCloud (Retrieve a webhook endpoint API) y se guarda en `integration_accounts.webhook_secret`. Implementación: `apps/motor-agente/src/lib/webhook-verify.ts`. Modo configurable vía `YCLOUD_WEBHOOK_VERIFY_MODE=disabled\|warn\|enforce` (default `warn`). En `enforce`, requests sin firma o con firma inválida → 401. Tolerance default 300s anti-replay. Para el tenant 7 el endpoint se creó vía API el 2026-08-25 y el secret real ya está guardado; queda en `warn` hasta ver el primer webhook real verificado. |
...
### V21 — la zona rechazada CIERRA en ese turno (2026-09-26)

En `packages/agent-pipeline/src/pipeline.ts` (mira el estado del Generator, no el texto, por eso no es regla de `shared-validator`). Con `zoneRejected`, el turno tiene que salir `disqualified` (o `handoff` B_derivacion / C_descualificado). Caso real: conv 12203 (+57), el motor declaraba el rechazo y el modelo siguió 24 mensajes porque la focal de fase le mandaba cualificar. Tres piezas: (1) la focal de fase se sustituye por la de cierre (`buildPhaseFocusInstruction(..., { zoneClose, zoneHandoff })`); (2) si el tenant tiene `lead_qualification.zone_close_message`, en modo cierre sale ESE literal tal cual (sin Judge ni Splitter) salvo que el modelo elija la excepción handoff B, y no se repite si ya se envió; (3) sin literal, un reintento y, si tampoco cierra, `ZoneCloseError`: `process-debounced` NO relanza (el cron reencolaría cada 30 s): pausa la IA y avisa a la entrenadora. Modo `handoff` (`zone.mode`): prefijo de fuera con residencia EN zona declarada en el formulario (D1 de Iván) → el turno tiene que ser handoff B.
...
**2026-09-26 — lista blanca** ([`docs/knowledge/project_tania_ronda_2026-09-26.md`](docs/knowledge/project_tania_ronda_2026-09-26.md)). `lead_qualification.zone_allowlist` = `{ always: [ISO], filtered: [ISO] }` (`apps/motor-agente/src/lib/zone-config.ts`, una sola fuente para formulario y chat); con ella, todo prefijo fuera de la lista (o desconocido con ≥8 dígitos) es `reject_by_prefix`, y `no_contact_countries` deja de mandar. Veredictos nuevos: `prefix_out_residence_in` (prefijo de fuera + formulario "vivo en zona" → handoff B), `reject_by_declaration` (sin teléfono, "vivo en / acá en / te escribo desde" + término de la lista → cierra). El cualificador del formulario lee la residencia (antes leía el campo del WhatsApp por el "país" de su etiqueta), aprueba en seco solo con país exacto de zona + prefijo de zona, y la IA pasa por una red que solo veta (`pais_iso` fuera de zona → rechazado); si la IA falla, decide el prefijo; un reenvío tras un rechazo con los criterios vigentes se rechaza 30 días.
