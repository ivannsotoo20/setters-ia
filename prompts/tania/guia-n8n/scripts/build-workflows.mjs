...
function buildPrincipal() {
  ...
  nodes.push(codeNode('normalizar', [220, 300], [
    ...
    "  out.origen = body.lead === 'Outbound' ? 'outbound' : 'inbound';",
  ...
  const composeLines = [
    ...
    "  'wa-outbound': null,          // << PEGAR AQUI prompts/tania/dist/tania-v4-wa-outbound.system.json",
    "  'wa-inbound-leadform': null,  // << PEGAR AQUI prompts/tania/dist/tania-v4-wa-inbound-leadform.system.json",
    '};',
    '',
    ...LIMPIEZA_LINES,
    '',
    ...
    '// si aun no existe, se decide por canal/origen/flujo de este payload.',
    'let variante = (crmRow && crmRow.fuente_v4) || null;',
    'if (!variante) {',
    "  if (norm.canal === 'whatsapp') variante = (norm.origen === 'inbound') ? 'wa-inbound-leadform' : 'wa-outbound';",
  ...
  nodes.push(sticky('Sticky — qué pegar dónde', [
    ...
    '1. **Nodo `compose`** → `DIST`: pegar el objeto completo de las 4 variantes de `prompts/tania/dist/`:',
    '   - `ig-inbound` ← `tania-v4-ig-inbound.system.json`',
    '   - `ig-bienvenidas` ← `tania-v4-ig-bienvenidas.system.json`',
    '   - `wa-outbound` ← `tania-v4-wa-outbound.system.json`',
    '   - `wa-inbound-leadform` ← `tania-v4-wa-inbound-leadform.system.json`',
    '   Hasta entonces el nodo lanza error a propósito. La variante se fija en `clientes_crm_tania.fuente_v4` al primer contacto; si es NULL se decide por canal/origen/flujo (el flow de bienvenida de ManyChat debe mandar `"flujo":"bienvenida"` en el body).',
