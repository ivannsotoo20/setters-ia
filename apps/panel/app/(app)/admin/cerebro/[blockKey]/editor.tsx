...
interface Props {
  blockKey: string;
  /** null = bloque global del Cerebro. */
...
export function CerebroBlockEditor(props: Props) {
  ...
  // Versión del histórico que el usuario está VIENDO en el textarea (puede ser
  // la activa, una vieja, o "draft" si modificó). NO equivale a la versión que
  // está aplicada en producción (esa es siempre props.activeVersion).
  ...
  const [previewTenantId, setPreviewTenantId] = useState<number | null>(
    props.tenants.find((t) => t.isActive)?.id ?? null,
  );
  ...
  const tenantsAffected = props.tenantId == null ? props.tenants.filter((t) => t.isActive).length : 1;

  // Resync cuando el server actualiza props (tras Apply/Discard → router.refresh
  // → server component recarga loadActiveBlock). Sin esto el textarea queda con
  // contenido stale tras un Apply.
  ...
  // Autosave debounced. Solo dispara cuando el contenido difiere del active.
  useEffect(() => {
    if (!isDirty) return;
  ...
  function handleSelectVersion(versionNumber: number) {
    ...
    // Si seleccionas la versión activa, evitamos un round-trip (ya tenemos el contenido en props).
    ...
    startLoadVersion(async () => {
      ...
      if (r.ok) {
        ...
        toast.success(`Cargada v${versionNumber}`, {
          description:
            r.changeSummary ??
            new Date(r.changedAt).toLocaleString('es-ES'),
        });
      } else {
  ...
  function handleApply() {
    startApply(async () => {
      // Si el draft está stale (acabamos de cambiar contenido y autosave aún no
      // ha disparado), forzamos un saveDraft inmediato antes del apply.
      if (autosaveTimer.current) {
        clearTimeout(autosaveTimer.current);
        autosaveTimer.current = null;
      }
  ...
  // Etiqueta del botón del dropdown según estado
            ...
            <Dialog open={applyDialogOpen} onOpenChange={setApplyDialogOpen}>
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Esto guardará el contenido actual del editor como la nueva versión activa de{' '}
                    <code className="text-xs font-mono">{props.blockKey}</code>. Afectará a{' '}
                    <strong>{tenantsAffected}</strong> tenant(s) productivo(s) en el siguiente turno
                    del motor.
                  </DialogDescription>
                </DialogHeader>
      ...
      <Card className="flex flex-col">
        <CardHeader>
          ...
          <CardDescription className="text-xs">
            Compone el prompt final usando el contenido actual del editor para el tenant + fase
            elegidos.
          </CardDescription>
        </CardHeader>
        <CardContent className="flex-1 flex flex-col gap-3">
          ...
          {preview ? (
            ...
            <div className="flex-1 min-h-[400px] flex items-center justify-center text-sm text-muted-foreground">
              Elige tenant + fase y click &quot;Generar preview&quot; para ver el prompt completo.
            </div>
          )}
        </CardContent>
      </Card>
